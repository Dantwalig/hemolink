...
export default function HospitalLoginPage() {
  ...
  const validateAll = () => {
    const hospitalIdErr = validators.hospitalId(form.hospitalId);
    const emailErr      = validators.email(form.email);
    const passwordErr   = validators.password(form.password);
    setErrors({ hospitalId: hospitalIdErr, email: emailErr, password: passwordErr });
    setTouched({ hospitalId: true, email: true, password: true });
    return !hospitalIdErr && !emailErr && !passwordErr;
  };
  ...
  const handleLogin = async () => {
    ...
    try {
       ...
       * localStorage.setItem("hemolink_hospital_token", data.token);
...
const styles = {
  ...
  // Left
  leftPanel:  { flex: 1, background: "#1C1C1C", display: "flex", alignItems: "center", justifyContent: "center", padding: "60px 48px", position: "relative", overflow: "hidden" },
  leftInner:  { position: "relative", zIndex: 1, maxWidth: 420, width: "100%" },
  ...
  hospitalIcon: { fontSize: 52, marginBottom: 20 },

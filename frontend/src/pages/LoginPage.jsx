...
export default function LoginPage() {
  ...
  const validate = (field, val) => {
    const err = validators[field] ? validators[field](val) : "";
  ...
  const handleLogin = async () => {
    ...
    try {
       ...
       * if (!response.ok) { setServerError(data.message || "Login failed."); return; }
       * localStorage.setItem("hemolink_token", data.token);
       * navigate("/donor/dashboard");
       */

      await new Promise((r) => setTimeout(r, 1400)); // simulated delay — remove when backend ready
      navigate("/donor/dashboard");
    } catch {
      ...
      <div style={styles.leftPanel}>
        <div style={styles.leftInner}>
          ...
          <div style={styles.leftCard}>
            <div style={styles.leftCardIcon}>🩸</div>
            <div>
        ...
        <div style={styles.formBox}>
          ...
          {forgotSent && (
            <div style={{ ...styles.alert, ...styles.alertSuccess }}>
              ✅ If your number is registered, you'll receive an SMS with reset instructions.
            </div>
          )}
          ...
          <p style={styles.switchLink}>
            Not registered yet?{" "}
            <button style={styles.linkBtn} onClick={() => navigate("/register")}>Create a donor account</button>
          </p>
          <p style={styles.switchLink}>
            Are you a hospital?{" "}
...
const styles = {
  ...
  leftCardIcon: { fontSize: 28, flexShrink: 0 },
  ...
  input:      { width: "100%", padding: "11px 14px 11px 38px", border: "1.5px solid #DDD5D0", borderRadius: 9, fontFamily: "'DM Sans', sans-serif", fontSize: 14, color: "#1C1C1C", outline: "none", transition: "border-color 0.2s, box-shadow 0.2s" },
  togglePw:   { position: "absolute", right: 12, background: "none", border: "none", cursor: "pointer", fontSize: 16, color: "#6B6B6B", padding: 4 },
  ...
  // Buttons
  submitBtn:  { width: "100%", padding: 13, background: "#C0392B", color: "#fff", border: "none", borderRadius: 10, fontFamily: "'DM Sans', sans-serif", fontSize: 15, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, marginTop: 4 },

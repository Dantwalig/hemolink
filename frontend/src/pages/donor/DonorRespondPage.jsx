...
/* ──────────────────────────────────────────────────────────────
   ...
   This page is opened when a donor clicks the link in their
   SMS alert. The URL will include a token, e.g.:
   /donor/respond?token=abc123
   
   The token is used to:
   1. Fetch the blood request details
   2. Submit the donor's response (Accept / Decline)
   ────────────────────────────────────────────────────────────── */
export default function DonorRespondPage() {
  const navigate       = useNavigate();
  const [searchParams] = useSearchParams();
  const token          = searchParams.get("token"); // from SMS link
  ...
  /* ──────────────────────────────────────────────────────────
     On mount: fetch request details using the token
     ────────────────────────────────────────────────────────── */
  useEffect(() => {
  ...
  const fetchRequestDetails = async () => {
    ...
    try {
       ...
       * }
       *
       * setRequestData(data);  // expects: { hospital_name, blood_type_code, units_needed, urgency_level, needed_by, distance_km }
       * setStatus("ready");
       */

      // Simulated data — remove when backend is ready
  ...
  const handleRespond = async (donorResponse) => {
    ...
    try {
      ...
      await new Promise((r) => setTimeout(r, 1400)); // simulated — remove when backend ready
      setStatus("success");
    } catch {
      setStatus("error");
    } finally {
  ...
  if (status === "error") return (
    <div style={styles.centeredPage}>
      ...
      <p style={styles.stateDesc}>
        ...
        If you received an SMS, please make sure you copied the full link.
      </p>
  ...
  if (status === "expired") return (
    <div style={styles.centeredPage}>
      ...
      <p style={styles.stateDesc}>
        This blood request has already been fulfilled or the response window has closed.
        Thank you for being a registered donor — you may be needed next time.
      </p>
      <button style={styles.btnOutline} onClick={() => navigate("/login")}>
        Go to My Dashboard
      </button>
  ...
  // Success — donor responded
  if (status === "success") return (
    <div style={styles.centeredPage}>
      <div style={styles.stateIcon}>{response === "Accepted" ? "✅" : "🙏"}</div>
      <h2 style={styles.stateTitle}>
        {response === "Accepted" ? "Thank you!" : "Response received"}
      </h2>
      <p style={styles.stateDesc}>
        {response === "Accepted"
      ...
      {response === "Accepted" && (
        <div style={styles.hospitalCard}>
          <div style={styles.hospitalCardIcon}>🏥</div>
          <div>
  ...
  /* ──────────────────────────────────────────────────────────
     MAIN — Request details + Accept / Decline
     ────────────────────────────────────────────────────────── */
  const urgencyColor = {
    Critical: "#C0392B",
    High:     "#E67E22",
    Medium:   "#F1C40F",
    Low:      "#1E8449",
  }[requestData?.urgency_level] || "#C0392B";

  const neededBy = requestData?.needed_by
    ? new Date(requestData.needed_by).toLocaleString("en-RW", {
      ...
      <div style={styles.content}>
        <div style={styles.card}>
          ...
          <p style={styles.cardSub}>
            A hospital near you needs your blood type urgently. Please review the details
            and let them know if you can help.
          </p>
          ...
          <div style={styles.detailsGrid}>
            ...
            <div style={styles.detailItem}>
              ...
              <span style={{ ...styles.detailValue, color: "#C0392B", fontSize: 22, fontWeight: 800 }}>
                🩸 {requestData?.blood_type_code}
              </span>
            </div>
            <div style={styles.detailItem}>
            ...
            <div style={styles.detailItem}>
              ...
              <span style={styles.detailValue}>📍 {requestData?.distance_km} km away</span>
            </div>
            <div style={styles.detailItem}>
            ...
            <div style={styles.detailItem}>
              ...
              <span style={{ ...styles.detailValue, color: urgencyColor, fontWeight: 700 }}>
          ...
          <div style={styles.responseRow}>
            <button
              ...
              disabled={loading}
            >
              {loading && response === "Accepted"
                ? <><span style={styles.spinner} />&nbsp;Confirming…</>
                : "✅ Yes, I can donate"}
            </button>
            <button
              style={{ ...styles.btnDecline, ...(loading ? styles.btnDisabled : {}) }}
              onClick={() => handleRespond("Declined")}
              disabled={loading}
            >
              {loading && response === "Declined"
          ...
          <p style={styles.noteText}>
            ℹ️ Your response will be sent to the hospital immediately. If you accept,
            please go to the hospital as soon as possible and mention HemoLink at reception.
          </p>
        </div>
      </div>
    </div>
...
const styles = {
  ...
  pulseDot:     { width: 8, height: 8, borderRadius: "50%", background: "rgba(255,255,255,0.9)", display: "inline-block", animation: "pulse 1.5s infinite" },
  ...
  // Centered state pages (loading, error, success)
  ...
  stateIcon:    { fontSize: 56, marginBottom: 20 },
  ...
  // Hospital card (on accept success)
  ...
  hospitalCardIcon: { fontSize: 28, flexShrink: 0 },

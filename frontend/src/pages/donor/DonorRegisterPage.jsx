...
export default function DonorRegisterPage() {
  ...
  const validate = (field, val) => {
    let err = "";
    if (field === "fullName")        err = validators.fullName(val);
    if (field === "phone")           err = validators.phone(val);
    if (field === "email")           err = validators.email(val);
    if (field === "bloodType")       err = validators.bloodType(val);
    if (field === "password")        err = validators.password(val);
    if (field === "confirmPassword") err = validators.confirmPassword(val, form.password);
    if (field === "consentSms")      err = validators.consent(val);
  ...
  const validateStep1 = () => {
    ...
    const touch2 = {};
    fields.forEach((f) => {
      errs[f]   = validators[f] ? validators[f](form[f]) : "";
      touch2[f] = true;
    });
    setErrors((e) => ({ ...e, ...errs }));
    setTouched((t) => ({ ...t, ...touch2 }));
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      await new Promise((r) => setTimeout(r, 1600)); // simulated — remove when backend ready
      setDone(true);
    } catch {
    ...
    <div style={styles.page}>
      <div style={styles.successPage}>
        ...
        <p style={styles.successDesc}>
          Welcome, <strong>{form.fullName.split(" ")[0]}</strong>! Your{" "}
          <strong>{form.bloodType}</strong> blood type is saved. You'll receive SMS alerts
          at <strong>{form.phone}</strong> when a nearby hospital needs you.
        </p>
    ...
    <div style={styles.page}>
      ...
      <div style={styles.leftPanel}>
        <div style={styles.leftInner}>
          ...
          <h1 style={styles.leftTitle}>
            Join the <em style={styles.leftEm}>network</em> that saves lives.
          </h1>
      ...
      <div style={styles.rightPanel}>
        <div style={styles.formBox}>
          ...
          {/* Server error */}
          {serverError && (
            <div style={{ ...styles.alert, ...styles.alertError }}>❌ {serverError}</div>
          )}
            ...
            <div>
              <div style={styles.fieldRow}>
                <Field label="Full Name" required error={touched.fullName && errors.fullName}>
                  <InputField
                    icon="" value={form.fullName} placeholder="Jean Claude Niyomugabo"
              ...
              <div style={styles.fieldRow}>
                <Field label="Email Address" error={touched.email && errors.email}
                  ...
                  <InputField
                    icon="" type="email" value={form.email} placeholder="jean@example.com"
              ...
              <div style={styles.availRow}>
                ...
                <label style={styles.toggleSwitch}>
                  ...
                  <span style={{
                    ...styles.toggleSlider,
                    background: form.available ? "#1E8449" : "#DDD5D0",
                  }}>
                    <span style={{
                      ...styles.toggleThumb,
                      transform: form.available ? "translateX(20px)" : "translateX(0)",
                    }} />
                  </span>
                </label>
              </div>

              <button style={styles.btnPrimary} onClick={goToStep2}>
                Continue →
              </button>

              <p style={styles.switchLink}>
                Already have an account?{" "}
                <button style={styles.linkBtn} onClick={() => navigate("/login")}>Log in here</button>
              </p>
            </div>
          )}
          ...
          {step === 2 && (
            <div>
              <Field label="Password" required error={touched.password && errors.password}
                ...
                <InputField
                  icon="🔒" type="password" value={form.password}
              ...
              <Field label="Confirm Password" required error={touched.confirmPassword && errors.confirmPassword}>
                <InputField
                  icon="🔒" type="password" value={form.confirmPassword}
              ...
              <div style={styles.consentRow}>
                ...
                <label htmlFor="consent" style={styles.consentLabel}>
                  I agree to receive <strong>SMS alerts</strong> when a hospital near me needs
                  blood that matches my type. I can opt out at any time.
                </label>
              </div>
              {touched.consentSms && errors.consentSms && (
                <span style={{ ...styles.errorMsg, display: "block", marginTop: -10, marginBottom: 14 }}>
                  ⚠ {errors.consentSms}
                </span>
              )}
...
const styles = {
  ...
  // Left panel
  leftPanel:    { flex: 1, background: "#C0392B", display: "flex", alignItems: "center", justifyContent: "center", padding: "60px 48px", position: "relative", overflow: "hidden" },
  ...
  stepCircle:   { width: 30, height: 30, borderRadius: "50%", border: "2px solid #DDD5D0", background: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 700, color: "#6B6B6B", transition: "all 0.3s" },
  stepActive:   { background: "#C0392B", borderColor: "#C0392B", color: "#fff" },
  stepDone:     { background: "#1E8449", borderColor: "#1E8449", color: "#fff" },
  stepLine:     { flex: 1, height: 2, background: "#DDD5D0", margin: "0 8px", marginBottom: 18, transition: "background 0.3s" },
  ...
  // Buttons
  btnPrimary:   { width: "100%", padding: 13, background: "#C0392B", color: "#fff", border: "none", borderRadius: 10, fontFamily: "'DM Sans', sans-serif", fontSize: 15, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, marginTop: 4 },
  btnOutline:   { padding: "13px 24px", background: "transparent", color: "#C0392B", border: "2px solid #C0392B", borderRadius: 10, fontFamily: "'DM Sans', sans-serif", fontSize: 15, fontWeight: 700, cursor: "pointer", marginTop: 4 },
  ...
  // Success screen
  successPage:  { margin: "auto", textAlign: "center", padding: "60px 40px", maxWidth: 480 },

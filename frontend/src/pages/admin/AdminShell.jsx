  ...
  return (
    <div style={{ display:"flex", height:"100vh", overflow:"hidden", fontFamily:"'Sora',sans-serif", background:"#FDF4F2" }}>
      <style>{SORA_FONT + SHARED_BTN_CSS + `
        ...
        .admin-nav-item:hover{background:rgba(192,57,43,.2);color:rgba(255,255,255,.85);}
        .admin-nav-active{background:rgba(192,57,43,.35) !important;color:#fff !important;font-weight:700 !important;box-shadow:inset 3px 0 0 #E87B6E;}
      ...
      {/* Sidebar */}
      <aside style={{ width:234, background:"linear-gradient(185deg,#1C0505 0%,#2A0A0A 40%,#0E0000 100%)", display:"flex", flexDirection:"column", padding:"24px 0", flexShrink:0, position:"sticky", top:0, height:"100vh", borderRight:"1px solid rgba(192,57,43,.2)", boxShadow:"4px 0 20px rgba(0,0,0,.4)" }}>
        <div style={{ display:"flex", alignItems:"center", gap:10, padding:"0 20px 18px", borderBottom:"1px solid rgba(192,57,43,.15)", marginBottom:6 }}>
          <LogoDrop size={30}/>
          <div>
            <div style={{ fontWeight:800, fontSize:15, color:"#fff" }}>Hemo<span style={{ color:"#E87B6E" }}>Link</span></div>
            <div style={{ fontSize:9, color:"#C0392B", textTransform:"uppercase", letterSpacing:1.2, fontWeight:700, marginTop:2, background:"rgba(192,57,43,.15)", padding:"2px 7px", borderRadius:4, display:"inline-block" }}>RBC Admin</div>
          </div>
        </div>

        {user?.email && (
          <div style={{ padding:"8px 20px 12px", borderBottom:"1px solid rgba(192,57,43,.1)", marginBottom:6 }}>
